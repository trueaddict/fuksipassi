<?php
  ...
  if (!mysqli_stmt_prepare($stmt, $sql2)) {
    ...
    if ($kayttaja = mysqli_fetch_assoc($result2)) {
      //echo "kayttaja ok".$_SESSION['usermail'];
    }
    else {
      //echo "kayttaja ei ok";
  ...
  if(isset($_POST['submit'])) {
    ...
    if(array_filter($errors)) {
      ...
      $sql2 = "UPDATE kayttajat SET suoritettu = '$suoritettu' WHERE mail = '$mail'";
...
<!DOCTYPE html>
<html lang="fi">
  <link rel="stylesheet" type="text/css" href="templates/dist/text-security.css">
  <?php include('templates/header.php') ?>
    <section class="container gray-text">
      <?php if($tehtava): ?>
      <h4 class="center">Kuittaa tehtävä <?php //echo htmlspecialchars($tehtava['id_teht_nro']); ?></h4>
      <h6 class="center"><?php echo htmlspecialchars($tehtava['kuvaus']); ?></h6>
      <form class="white kuittaus" autocomplete="off" action="check.php?id_teht_nro=<?php echo $tehtava['id_teht_nro']?>" method="POST">
        <p class="inline">Anna kuittaus salasana:</p>
        <input id="pass" class="pass" type="text" name="password" value="<?php echo htmlspecialchars($password) ?>">
